...
struct RigidBodyGroundCollisionConstraintSolver
{
    ...
    )
    {
        ...
        const Collision::CollisionShapeParams& shape_params = energies.data[c_idx].shape_params;
        ...
        if (shape_params.type == Collision::CollisionGeometryType::Sphere)
        {
            Vec3r cp_global = particles.positions[p_idx] - Vec3r(0, shape_params.sphere.radius, 0);
            C_n = n.dot(cp_ground - cp_global);
        }
        else if (shape_params.type == Collision::CollisionGeometryType::RodSegment)
        {
    ...
    : CollisionConstraintEnergySolver<RigidBodyGroundCollisionEnergyPool, RigidBodyGroundCollisionConstraintSolver>
{
    ...
    )
    {
        ...
        const Collision::CollisionShapeParams& sdf_params = energies.data[c_idx].shape_params;
        if (sdf_params.type == Collision::CollisionGeometryType::Sphere)
        {
            energies.data[c_idx].cp_rb_local = particles.rotation(p_idx).conjugate() * Vec3r(0, -sdf_params.sphere.radius, 0);
        }
        else if (sdf_params.type == Collision::CollisionGeometryType::RodSegment)
        {
