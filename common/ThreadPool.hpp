 ...
 *
 * @brief `BS::ThreadPool`: a fast, lightweight, modern, and easy-to-use C++17/C++20/C++23 thread pool library. This header file contains the entire library, and is the only file needed to use the library.
 */
...
import std;
    #else
        #error "The thread pool library cannot import the C++ Standard Library as a module using `import std` if the library itself is not imported as a module. Either use `import BS.ThreadPool` to import the library, or remove the `BS_THREAD_POOL_IMPORT_STD` macro. Aborting compilation."
    #endif
#else
    #undef BS_THREAD_POOL_IMPORT_STD
 ...
 */
inline constexpr version ThreadPool_version(BS_THREAD_POOL_VERSION_MAJOR, BS_THREAD_POOL_VERSION_MINOR, BS_THREAD_POOL_VERSION_PATCH);
...
static_assert(ThreadPool_version == version(BS_THREAD_POOL_MODULE), "The versions of BS.ThreadPool.cppm and BS_ThreadPool.hpp do not match. Aborting compilation.");
/**
 * @brief A flag indicating whether the thread pool library was compiled as a C++20 module.
 */
inline constexpr bool ThreadPool_module = true;
#else
/**
 * @brief A flag indicating whether the thread pool library was compiled as a C++20 module.
 */
inline constexpr bool ThreadPool_module = false;
#endif
 ...
 */
inline constexpr bool ThreadPool_import_std = true;
#else
/**
 * @brief A flag indicating whether the thread pool library imported the C++23 Standard Library module using `import std`.
 */
inline constexpr bool ThreadPool_import_std = false;
#endif
 ...
 */
inline constexpr bool ThreadPool_native_extensions = true;
#else
/**
 * @brief A flag indicating whether the thread pool library's native extensions are enabled.
 */
inline constexpr bool ThreadPool_native_extensions = false;
#endif
...
/**
 * @brief An enumeration class of flags to be used in the bitmask template parameter of `BS::ThreadPool` to enable optional features.
 */
enum class tp : opt_t
{
    /**
...
#ifdef __cpp_exceptions
/**
 * @brief An exception that will be thrown by `wait()`, `wait_for()`, and `wait_until()` if the user tries to call them from within a thread of the same pool, which would result in a deadlock. Only used if the flag `BS::tp::wait_deadlock_checks` is enabled in the template parameter of `BS::ThreadPool`.
 */
...
public:
    /**
     * @brief Get the index of the current thread. If this thread belongs to a `BS::ThreadPool` object, the return value will be an index in the range `[0, N)` where `N == BS::ThreadPool::get_thread_count()`. Otherwise, for example if this thread is the main thread or an independent thread not in any pools, `std::nullopt` will be returned.
     *
    ...
    /**
     * @brief Get a pointer to the thread pool that owns the current thread. If this thread belongs to a `BS::ThreadPool` object, the return value will be a `void` pointer to that object. Otherwise, for example if this thread is the main thread or an independent thread not in any pools, `std::nullopt` will be returned.
     *
     * @return An `std::optional` object, optionally containing a pointer to a thread pool. Note that this will be a `void` pointer, so it must be cast to the desired instantiation of the `BS::ThreadPool` template in order to use any member functions.
     */
 ...
 */
using light_ThreadPool = ThreadPool<tp::none>;
 ...
 */
using priority_ThreadPool = ThreadPool<tp::priority>;
 ...
 */
using pause_ThreadPool = ThreadPool<tp::pause>;
 ...
 */
using wdc_ThreadPool = ThreadPool<tp::wait_deadlock_checks>;
...
public:
     ...
     */
    explicit ThreadPool(const std::size_t num_threads) : ThreadPool(num_threads, [] {}) {}
...
private:
     ...
     */
    [[nodiscard]] static std::size_t determine_thread_count(const std::size_t num_threads) noexcept(!ThreadPool_native_extensions)
    {
        if (num_threads > 0)
            return num_threads;
#ifdef BS_THREAD_POOL_NATIVE_EXTENSIONS
